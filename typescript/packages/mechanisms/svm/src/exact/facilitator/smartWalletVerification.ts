  ...
  if (budget?.version === 1) {
    for (const ix of instructions) {
      if (ix.programAddress.toString() === COMPUTE_BUDGET_PROGRAM_ADDRESS.toString()) {
        throw new Error(
          "smart_wallet_unsupported_compute_budget_instruction: version 1 transactions must use message.config",
        );
      }
    }
    ...
    if (violation === "compute_unit_limit_missing") {
      throw new Error(
        "smart_wallet_compute_unit_limit_missing: version 1 transactions must set config.computeUnitLimit",
      );
    }
